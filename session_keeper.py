...
class SessionKeeper:
    ...
    def _discover_anon_key(self):
        ...
        Support format lama (JWT eyJhbGci...) dan format baru (sb_publishable_...)."""
        if self._anon_key:
            return
        ...
        try:
            ...
            new_fmt = re.compile(r'sb_publishable_[A-Za-z0-9_\-]+')
            m = new_fmt.search(r.text)
            if m:
                self._anon_key = m.group(0)
                log.info(f"  ✅ Supabase anon key ditemukan (format baru)!")
                return
            ...
            for m in old_fmt.finditer(r.text):
                jwt = m.group(0)
                parts = jwt.split(".")
                if len(parts) != 3:
                    continue
                try:
                    ...
                    if payload.get("role") == "anon" and payload.get("iss") == "supabase":
                        self._anon_key = jwt
                        log.info(f"  ✅ Supabase anon key ditemukan (format lama)!")
                        return
                except Exception:
                    continue
