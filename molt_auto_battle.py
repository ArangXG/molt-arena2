...
def step2_run(battle_id: str) -> bool:
    """Jalankan battle — butuh cookie session, sama seperti browser."""
    ...
    try:
        ...
        log.debug(f"  POST /run -> {r.status_code}")
        if r.status_code in (200, 201):
            return True
...
def step5_wait_final(battle_id: str, max_wait: int = 180) -> dict | None:
    """
    Polling setelah fase voting selesai.
    Winner baru di-set server setelah timer voting habis → status 'completed'.
    """
    final_status = {"completed", "finished", "done", "ended"}
    log.info(f"  ⏳ Menunggu hasil final (max {max_wait}s)...")
    elapsed = 0
    while elapsed < max_wait:
        time.sleep(10)
        elapsed += 10
        ...
        log.info(f"  ⌛ [{status.upper()}] winnerId={winner is not None} | votes={vote_a}:{vote_b} | +{elapsed}s")
        if status in final_status and winner is not None:
            log.info("  ✅ Hasil final diterima!")
            return battle
        # Kadang status tetap 'voting' tapi winnerId sudah ada
        if winner is not None:
            log.info("  ✅ WinnerId sudah ada → hasil final!")
            return battle
    log.warning("  ⚠️  Timeout tunggu hasil final — tampilkan data terakhir")
    data = api_get(f"/battles/{battle_id}")
    ...
    while True:
        ...
        if not r1 or r1.get("_error"):
            ...
            if s in (401, 403):
                ...
                log.warning(f"  ⚠️  Gagal buat battle (HTTP {s}) → skip")
                stats["skip"] += 1
            ...
            if result:
                ...
                if status_now == "voting":
                    ...
                    step4_vote(battle_id, AGENT_ID)
                    # Step 5: Tunggu hasil final setelah voting timer habis
                    log.info("  🏁 Step 5: Tunggu hasil final...")
                    final = step5_wait_final(battle_id)
